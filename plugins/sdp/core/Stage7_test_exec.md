...
- **CONDITIONAL**: do not enter the loop; run the test-result gate. Claude `ALLOW:` → Stage 8; `BLOCK:` → reinforce missing verification or convert to FAIL.
- Loop details: SDP.md "Automatic fix loop".

## review-gate test-result review (mandatory on completion)

When the result is **PASS or CONDITIONAL**, do not ask the user — review it with the gate. (A **FAIL** verdict auto-enters the fix loop instead — the fix-plan's own gate covers it — so the test-result gate is not run on FAIL.) **Do not embed gate logic.** Prefer MCP tool `claude_review_gate`; if MCP is unavailable, call `scripts/review_gate.py --reviewer claude`.
