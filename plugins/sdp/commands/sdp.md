...
- **Gate mode** comes from the anchor-selected `gates.yaml` (`mode:`), never from the environment; with no config at any discovery tier the built-in default is `unattended`. On `INFRA_ERROR`, report the Claude/agy tooling failure before any stage advance. Note a user-global `gates.yaml` sets the mode for every project that ships no local override.
