...
| `scripts/review_gate.py` | The gate engine (~2.4k lines). Standalone-callable. |
| `scripts/sdp_mcp_server.py` | MCP server exposing the gate to Codex. |
| `.sdp/` | This repo's own SDP config — SDP dogfoods itself. |
| `tests/` | 14 suites, ~350 assertions. `tests/run.sh` is the single entry point. |
...
**Working, pre-1.0.** The plugin installs and runs on both hosts, the gate is exercised by ~350 assertions across 14 suites, and this repo dogfoods SDP on itself. Interfaces (config keys, marker grammar, gate CLI) may still change before 1.0 — pin `sdp_version` if you depend on them.
