...
| Background problems | (a) `scripts/sop/` relative paths hardcoded; (b) `base_dir` forks per project — two different legacy deliverable roots were in use; (c) command-name drift across 5 names (`/sop`·`/batch-sop`·`/meta-sdp`·`/meta-workflow`·`/worktree-dispatch`); (d) **real drift bug**: `agy-gate-fallback.sh` exists only in Project-A → the other 5 projects unconditionally Fail-Close BLOCK when codex does not run. |
...
| base_dir | Single root for deliverables·gate logs·halt·thread files. **Common default = `${CLAUDE_PROJECT_DIR}/.private/sdp-artifacts`** (project-overridable). Replaces the legacy per-project deliverable root. |
...
|---|---|---|
| 4.1 | Core is separable (SOP.md skeleton, 500-char bridge summary, 12× fix loop, Agent Team, gate bash) | `run_segment_tmux.sh` lived in a different source directory from the rest of the SOP assets. |
| 4.2 | **The core itself is polluted with Project-A domain rules** | The source SOP body carries project-specific domain literals — a DB column definition, a migration-filename convention, and build and server-start commands. Splitting config is insufficient → **de-domain the body text**. |
...
| REQ-U-02 | Remove domain sentences (DB column definitions, migration filenames, build and server-start commands, etc.) from SDP.md/Stage*.md body → externalize to config references or a **project rules addendum**. | M | 6-project regression passes |
...
| REQ-U-06 | **Unified deliverable root**: base_dir default = `${CLAUDE_PROJECT_DIR}/.private/sdp-artifacts`. Tree = `${base_dir}/{YYYY-MM-DD}/{topic}/{doc-type}` (date→topic→doc-type, inheriting Project-A's organization). Replaces the two legacy per-project deliverable roots with this single default (project-overridable). | M | Deliverable path convention |
...
- **Correction**: req #6 is not net-new but a **consolidation + cadence parameterization** of what Project-A's phase-2 planning document already implements. Current forces every round (7–12) (`CONSENSUS_REACHED` marker); the requirement is even rounds only.
