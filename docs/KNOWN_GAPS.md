...
**This section carries 27 rows, `NC-01` .. `NC-27`.**
...
| **NC-26** | **`orca` dispatch mode (`dispatch.worktree_mode: orca`) has never been driven by `worktree-dispatch` itself.** Six workers have been run against the adapter's own call sequence — one canary plus five concurrent — which measured: the base is pinned when a full SHA is passed; a 2092-char handover reaches the worker byte-identical inside `--spec`; `--name` determines both the worktree directory and the branch `<repo.gitUsername>/<name>`; the worktrees are linked worktrees of the main repo; five ran genuinely concurrently with no cross-delivery and no queueing; one failing task did not disturb the other four; `orca worktree rm` preserves a branch that carries commits. What remains **unexercised is the command path**: no main session has run Stage 1, written segment handovers, and driven the adapter end to end, so the integration step — main merging each `result_sha` — has never been performed. Also still open: `--timeout-ms` semantics are unobserved; the permission boundary under a privileged action was never exercised, so **`orca` must not be claimed to avoid `auto`'s `bypassPermissions` requirement**; a worker's setup-hook sequencing is untested because **no registered repository uses `wait-for-setup`** (of 20, two define a hook and both are `start-immediately`) — a real risk that is simply unreachable here; and crash/restart reconciliation between `task-create` and `worker-start` is untested. The version allow-list is a single verified `(appVersion, schemaVersion)` pair — `(1.4.176, 1)`, hardcoded in `_version_allowed` — so an ordinary Orca upgrade silently disables the mode until fresh fixtures widen the list; deliberate (a verified allow-list, not a floor), but an operational surprise. Unlike `auto`, which owns the tmux session it spawns and enforces `dispatch.default_timeout` as a hard cap (NFR-05), `orca` does not own the worker's process and enforces **no** NFR-05-equivalent hard time cap — past the configured deadline `status` exits `124` meaning over budget and still alive, and nothing is stopped. |
